...
describe('Model Persistence & Import/Export', () => {
  ...
  describe('Performance and Memory', () => {
    it('should handle large model exports efficiently', () => {
      ...
      const largeDt = new DecisionTree('target', ['feature1', 'feature2', 'feature3']);
      largeDt.train(largeData);
      
      // Export should not crash
      const exported = largeDt.toJSON();
      assert.ok(exported);
      // Note: Current implementation doesn't store training data
      assert.strictEqual(exported.data.length, 0);
      
      // Import should work
      const importedDt = new DecisionTree(exported);
      const prediction = importedDt.predict({ feature1: 'value1', feature2: 'value2', feature3: 'value1' });
      assert.ok(typeof prediction === 'string');
    });

    it('should handle deep tree model exports', () => {
      // Create a deep tree
      const deepData = [];
      for (let i = 0; i < 100; i++) {
      ...
      const deepDt = new DecisionTree('target', ['level1', 'level2', 'level3', 'level4', 'level5', 'level6', 'level7']);
      deepDt.train(deepData);
      
      // Export should work
      const exported = deepDt.toJSON();
      assert.ok(exported);
      
      // Import should work
      const importedDt = new DecisionTree(exported);
      const prediction = importedDt.predict({ level1: '1', level2: '0', level3: '1', level4: '0', level5: '1', level6: '0', level7: '1' });
      assert.ok(typeof prediction === 'string');
    });
  });
});
